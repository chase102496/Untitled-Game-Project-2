...
func _ready() -> void:
	...
	abil.current_status_effects.add_passive(abil.status_weakness.new(self,Battle.type.CHAOS))
	abil.current_status_effects.add_passive(abil.status_immunity.new(self,Battle.type.VOID))
