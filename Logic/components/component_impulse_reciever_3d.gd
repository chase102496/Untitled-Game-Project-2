...
## HACK this is what we modified from filters
...
func _on_my_impulse_activated() -> void:
	_set_load_state("_on_my_impulse_activated_load")
	Debug.message([name," _on_my_impulse_activated"],Debug.msg_category.WORLD)
	
	_emit("activated")
	_state_transition.call_deferred("activated")

func _on_my_impulse_deactivated() -> void:
	_set_load_state("_on_my_impulse_deactivated_load")
	Debug.message([name," _on_my_impulse_deactivated"],Debug.msg_category.WORLD)
	
	_emit("deactivated")
	_state_transition.call_deferred("deactivated")

func _on_my_impulse_activated_load() -> void:
	Debug.message([name," _on_my_impulse_activated_load"],Debug.msg_category.WORLD)
	_set_load_state("_on_my_impulse_activated_load")
...
func _on_my_impulse_deactivated_load() -> void:
	Debug.message([name," _on_my_impulse_deactivated_load"],Debug.msg_category.WORLD)
	_set_load_state("_on_my_impulse_deactivated_load")
	
	_emit("deactivated")
	_state_transition.call_deferred("deactivated")

## HACK Internal Output
